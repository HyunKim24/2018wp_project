...
module.exports = function(passport) {
  ...
  passport.use('local-signin', new LocalStrategy({
    ...
    try {
      ...
      if (user && await user.validatePassword(password)) {
        return done(null, user, req.flash('success', '로그인 되었습니다. 오늘도 열심히 사는 당신 힘내세요! 오늘도 열심히 사는 당신 힘내세요!'));
      }
  ...
  const callbackURL = (process.env.NODE_ENV == 'production')?
    'https://sheltered-stream-87144.herokuapp.com/auth/facebook/callback':
    'https://localhost:3000/auth/facebook/callback';

  passport.use(new FacebookStrategy({
    // 이 부분을 여러분 Facebook App의 정보로 수정해야 합니다.
    clientID : process.env.FBID || '481570702334607',
    clientSecret : process.env.FB_SECRET || '63fd3eb70afed66705761e0db6bf1f82',
